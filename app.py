...
def get_admin_credentials():
    ...
    if not username or not password:
        # In production, we must have these. If missing, login will fail.
...
# Admin Notification Configuration - loaded from environment variables
def get_admin_notifications_config():
    """Load admin-specific notification settings"""
    return {
        'telegram': {
...
def send_admin_notification(subject, message_text, html_body=None):
    """Send separate notifications to admin via Telegram and SMTP"""
    ...
    if tg_token and tg_chat:
        try:
            ...
            app.logger.error(f"Admin Telegram Error: {str(e)}")

    # 2. SMTP Admin Notification
    smtp_cfg = config['smtp']
    if all([smtp_cfg['host'], smtp_cfg['email'], smtp_cfg['password']]):
        try:
            ...
            app.logger.error(f"Admin SMTP Error: {str(e)}")

def send_user_notification(username, subject, message_text, html_body=None):
    """Send notifications to specific user via their Telegram and SMTP settings"""
    ...
    if tg_token and tg_chat:
        try:
            ...
            # Match the admin's logic: direct threading with requests.post
            threading.Thread(target=lambda: requests.post(url, json=payload, timeout=10)).start()
        except Exception as e:
            app.logger.error(f"User Telegram Error ({username}): {str(e)}")

    # 2. SMTP User Notification
...
def load_telegram_config():
    ...
    # CRITICAL: Global config for admin should ONLY come from environment variables
    bot_token = os.environ.get('ADMIN_TELEGRAM_BOT_TOKEN', '')
    chat_id = os.environ.get('ADMIN_TELEGRAM_CHAT_ID', '')
...
def get_telegram_credentials(username=None):
    """Get Telegram credentials - user-specific only for user notifications"""
    if username:
    ...
    # For user notifications, we MUST NOT fall back to global
    return None, None
...
def reset_demo_data():
    """Reset demo data to default state for Live Demo Edition - Disabled for production safety"""
    pass
...
def save_message(name,
    ...
    new_message = {
        ...
        'recipient': username,
        'priority': priority,
        'category': 'portfolio' if username != ADMIN_CREDENTIALS['username'] else 'platform'
    }
...
<i>Advice: Respond promptly to maintain professional engagement.</i>
"""
        send_user_notification(username, "New Portfolio Message", notification_msg, html_body=None)
    else:
        notification_msg = f"""
🛡️ <b>New Platform Inquiry</b>
...
{message}
"""
        send_admin_notification("Platform Inquiry", notification_msg)
...
def track_visitor(username=None):
    ...
    if visitor_ip not in today_ips:
        ...
        # Notify admin of new unique visitor
        send_admin_notification(
            "New Visitor Tracked",
            f"👤 <b>New unique visitor</b>\n🌐 IP: {visitor_ip}\n🎯 Target: {username}\n⏰ Time: {datetime.now().strftime('%H:%M:%S')}"
        )
...
def contact_academy():
    ...
    try:
        ...
        send_admin_notification("Academy Inquiry", tg_msg)
...
def contact():
    ...
    try:
        ...
        # TRIGGER NOTIFICATION: Match the logic of send_admin_notification
        # Prepare the notification message for the user
        company_info = f"Company: {company}\n" if company else ""
        user_notification_text = f"📨 New Portfolio Message\n\nName: {name}\n{company_info}Email: {email}\nType: {request_type}\nArea: {interest_area}\n\nMessage:\n{message_content}"
        
        # Determine the correct recipient username for notification
        # The portfolio_owner variable is what we should use
        recipient_username = portfolio_owner
        
        if recipient_username != 'admin':
            app.logger.info(f"NOTIFICATION_TRIGGER: Sending to user {recipient_username}")
            send_user_notification(recipient_username, "New Portfolio Message", user_notification_text)
        else:
            app.logger.info("NOTIFICATION_TRIGGER: Sending to admin")
            send_admin_notification("Platform Inquiry", user_notification_text)
...
def dashboard_settings():
    ...
    telegram_bot_token_display = (bot_token[:10] + '...' + bot_token[-5:]) if len(bot_token) > 15 else '********'
...
def dashboard_messages():
    ...
    try:
        if is_admin:
            # Admin sees platform messages or internal messages
            # portfolio messages are hidden from admin (they belong to users)
            if category == 'all':
                ...
                # Return empty for admin if explicitly requesting portfolio
                db_messages = []
        else:
...
def dashboard_delete_message(message_id):
    ...
    # 1. Attempt to delete from Database (Synchronized approach)
    try:
        db_msg = Message.query.get(message_id)
        if db_msg:
            # Security: If not admin, can only delete if they are the receiver
            if is_admin or str(db_msg.receiver_id) == str(session.get('user_id')):
                db.session.delete(db_msg)
                db.session.commit()
                app.logger.info(f"Message {message_id} deleted from DB by {username}")
    except Exception as e:
        app.logger.error(f"DB Deletion Error: {str(e)}")
        db.session.rollback()

    # 2. Delete from JSON (Legacy/Fallback storage)
    try:
        data = load_data(username=username)
        ...
        if len(data['messages']) < original_count:
            save_data(data, username=username)
            app.logger.info(f"Message {message_id} deleted from JSON for {username}")
            flash('Message deleted successfully', 'success')
        else:
            # If deleted from DB but not found in JSON, it's still a success
            flash('Message deleted successfully', 'success')
            
    except Exception as e:
        app.logger.error(f"JSON Deletion Error: {str(e)}")
        flash('Error updating records.', 'danger')
            ...
            if 'users' in data:
                for user in data['users']:
                    if user.get('username') == username:
                        ...
                        send_admin_notification(
                            "Login Activity",
                            f"🔑 <b>Login Successful</b>\n👤 User: {username}\n🛡️ Admin: {'Yes' if user.get('role') == 'admin' else 'No'}\n⏰ Time: {datetime.now().strftime('%H:%M:%S')}"
                        )
